...
RUN apk --no-cache add g++ gcc make cmake git nano libcurl python3 python3-dev \
    curl bash curl-dev linux-headers sqlite-dev sed tini
...
RUN git clone -b "${UPSTREAM_VER}" --single-branch --progress --depth 1 \
    ...
    cd trueblocks-core && \
    cat src/CMakeLists.txt | grep -v "examples" | grep -v dev_tools | grep -v testRunner >x && \
    cat x >src/CMakeLists.txt && \
    rm -f x && \
    mkdir -p build && \
    # Develop fix
    bash ./scripts/make-go-work.sh && \
    # end develop fix
    cd build && \
    cmake ../src && \
    make -j 5
